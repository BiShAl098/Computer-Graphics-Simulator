...
struct Car
{
    ...
    void update(float dt, const TrafficLight& light, const Car* otherCar = nullptr)
    {
        ...
        bool carAhead = false;
        float safeDistance = 100.0f;   // maintain 100px gap between car centers (increased from 80)
        ...
        if (carAhead)
        {
            // CAR AHEAD — brake hard to avoid collision
            // If they're slower or stopped, we need to slow down MORE than just matching
            float gap = otherCar->x - x;
            float dangerZone = (width + otherCar->width) / 2.0f + 50.0f;   // critical distance

            if (gap < dangerZone)
            {
                // Very close! Emergency brake to stop
                velocity -= CAR_BRAKE * dt * 2.0f;   // brake TWICE as hard
                if (velocity < 0.0f)
                    velocity = 0.0f;
            }
            else if (otherCar->velocity < velocity)
            {
                // They're slower — brake to match their speed (or slower if needed)
